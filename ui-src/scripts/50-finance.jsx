    ...
    function A4DocModal({ doc, docType, recId, currentUser, autoPrint, onClose }) {
      ...
      const injectEditStyles = (docObj) => {
        ...
        if (!style) {
          ...
          style.textContent = `
            ...
            body[contenteditable="true"] div:hover,
            body[contenteditable="true"] span:hover,
            body[contenteditable="true"] b:hover,
            body[contenteditable="true"] h1:hover,
            body[contenteditable="true"] h2:hover,
            body[contenteditable="true"] h3:hover {
      ...
      const toggleEdit = () => {
        ...
        if (docObj && docObj.body) {
          ...
          docObj.designMode = next ? 'on' : 'off';
          if (next) {
            docObj.body.addEventListener('input', () => setIsEdited(true), { once: true });
            docObj.body.focus();
            Swal.fire({
              icon: 'info',
              title: 'Chế độ Chỉnh sửa trực tiếp',
              text: 'Bạn có thể nhấp chuột trực tiếp vào bất kỳ dòng chữ, số tiền, tên người hoặc điều khoản nào để sửa đổi trước khi in.',
              timer: 2500,
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                {isEditing && (
                  <span className="badge badge-warning" style={{ fontSize: 11, padding: '3px 8px' }}>
                    <i className="fas fa-pen-to-square"></i> Đang chỉnh sửa
                  </span>
                )}
                {isEdited && (
                  <span className="badge badge-info" style={{ fontSize: 11, padding: '3px 8px' }}>
                    <i className="fas fa-check"></i> Đã tùy biến
                  </span>
                )}
              </div>
              <button className="close-btn" onClick={onClose}>&times;</button>
            ...
            <div className="modal-body" style={{ padding: '12px 16px' }}>
              ...
              <div className="mob-doc-modal-actions" style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                >
                  ...
                  {isEditing ? 'Đang bật sửa văn bản' : 'Chỉnh sửa trực tiếp'}
                </button>

                {isEdited && (
                  <button className="btn btn-secondary btn-sm" onClick={resetDoc} title="Đặt lại về văn bản gốc">
                    <i className="fas fa-rotate-left"></i> Khôi phục gốc
                  </button>
                )}

                <button className="btn btn-primary btn-sm" onClick={doPrint} style={{ fontWeight: 700 }}>
                ...
                <div style={{ marginLeft: 'auto', fontSize: 12, color: '#64748b', display: 'flex', alignItems: 'center', gap: 6 }}>
                  <i className="fas fa-circle-info" style={{ color: 'var(--navy-accent)' }}></i>
                  <span>{isEditing ? 'Nhấp chuột vào bất kỳ chữ/số nào trên văn bản để sửa' : 'Chuẩn khổ in A4'}</span>
                </div>
              </div>
    ...
    function ContractTemplatesManagerModal({ currentUser, onClose }) {
      ...
      const templates = (tplRes && tplRes.success && tplRes.data && tplRes.data.templates) || [
      ...
      const [busy, setBusy] = useState(false);
      const curTpl = templates.find((t) => t.key === selectedKey) || templates[0] || {};
      ...
      useEffect(() => {
        if (curTpl) {
          setTitle(curTpl.label || '');
          setShortLabel(curTpl.shortLabel || '');
          setHint(curTpl.hint || '');
          setCustomTerms(curTpl.customTerms || '');
        }
      }, [curTpl.key, curTpl]);

      const MERGE_TAG_GROUPS = [
        {
          group: '👤 Bên A (Chủ nhà / Bên chuyển nhượng)',
          tags: [
                  ...
                  {templates.map((t) => (
                    ...
                    >
                      ...
                      <div className="tpl-nav-info">
                        ...
                        <div className="tpl-nav-desc">{t.src === 'ten' ? 'Nguồn: HĐ Thuê' : 'Nguồn: Giao dịch'}</div>
                      </div>
                      {t.isCustom && <span className="tpl-nav-badge">Tùy biến</span>}
                    </div>
                  ))}
                  <div style={{ marginTop: 'auto', paddingTop: 10 }}>
                    <button className="btn btn-secondary btn-sm" style={{ width: '100%', fontSize: 11.5 }} onClick={handleReset}>
                  ...
                  <div className="tpl-edit-card">
                    ...
                    <div style={{ marginBottom: 14 }}>
                      ...
                      <input
                        ...
                        placeholder="Mô tả ngắn gọn hướng dẫn người dùng..."
                      />
                    </div>
