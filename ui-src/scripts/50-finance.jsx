                ...
                <div key={t.id} className={'mob-tenancy-card status-' + (t.status || '').toLowerCase()}>
                  ...
                  <div className="mob-tenancy-actions">
                    ...
                    <button className="mob-btn mob-btn-zalo" onClick={() => onAction('wa', t)} title="Nhắn Zalo khách thuê">
                      <svg className="zalo-logo-img" viewBox="0 0 100 100" style={{ width: 16, height: 16 }}>
                        <circle cx="50" cy="50" r="47" fill="#ffffff" stroke="#008fe5" strokeWidth="4.5" />
                        <path d="M 50 15 C 69.33 15 85 30.67 85 50 C 85 69.33 69.33 85 50 85 C 44.2 85 38.7 83.6 33.8 81.1 L 18 86.5 L 22.8 72.3 C 17.9 66.2 15 58.4 15 50 C 15 30.67 30.67 15 50 15 Z" fill="#008fe5" />
                        <text x="50.5" y="58" fill="#ffffff" fontFamily="system-ui, sans-serif" fontSize="28" fontWeight="900" textAnchor="middle" letterSpacing="-1.2">Zalo</text>
                      </svg>
...
function ContractTemplatesManagerModal({ currentUser, onClose }) {
  ...
  // Khởi tạo form khi dữ liệu máy chủ tải về lần đầu
  useEffect(() => {
    if (isInitialLoadRef.current && tplRes) {
  ...
  // Chỉ reset form khi người dùng CHỦ ĐỘNG CLICK CHỌN MẪU KHÁC ở cột 1
  useEffect(() => {
    if (prevSelectedKeyRef.current !== selectedKey) {
      ...
      const breakCount = useMemo(() => {
        const matches = (fullTemplateHtml || '').match(/tpl-page-break-gap|tpl-page-break-tag/g);
        return matches ? matches.length : 0;
      }, [fullTemplateHtml]);

      const totalA4Pages = Math.max(pageCount, breakCount + 1);
      const paperMinHeight = totalA4Pages * 1123 + breakCount * 44;

      useEffect(() => {
        const calculatePages = () => {
          if (richEditorRef.current) {
            const h = richEditorRef.current.scrollHeight || 0;
            const p = Math.max(1, Math.ceil(h / 1050));
            setPageCount(Math.max(p, breakCount + 1));
          }
        };
        calculatePages();
        const t = setTimeout(calculatePages, 200);
        return () => clearTimeout(t);
      }, [fullTemplateHtml, templateMode, breakCount]);
              ...
              if (docXmlFile) {
                ...
                if (htmlOutput) {
                  ...
                  Swal.fire({
                    ...
                    text: 'Văn bản đã được trích xuất hoàn chỉnh vào trang in A4!',
                    timer: 1800,
                  ...
                  <div className="tpl-canvas-workspace">
                    {templateMode === 'terms' ? (
                      ...
                      <div className="tpl-full-canvas-wrapper">
                        ...
                        <div className="tpl-a4-sheet-paper-container">
                          <div className="tpl-a4-sheet-paper" style={{ minHeight: `${paperMinHeight}px` }}>
                            <div
                              ref={richEditorRef}
