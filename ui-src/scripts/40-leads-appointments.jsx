    ...
    function FollowUpsView({ currentUser, role, perms, initialSearch }) {
      ...
      const FU_STAGES = [
        { key: '', label: 'Tất cả' },
        { key: 'Due Now', label: 'Cần làm ngay' },
      ...
      const counts = useMemo(() => { const o = { '': base.length }; base.forEach((f) => { const s = stageOf(f); o[s] = (o[s] || 0) + 1; }); return o; }, [base]);
      const visible = useMemo(() => (stage ? base.filter((f) => stageOf(f) === stage) : base), [base, stage]);
      const activeFiltersCount = (filters.search ? 1 : 0) + (filters.type ? 1 : 0) + (filters.agent ? 1 : 0) + (stage ? 1 : 0);

      const kpi = useMemo(() => { const r = rows || []; return [
        [r.filter((f) => f.status === 'Pending').length, 'Chờ xử lý', 'fa-hourglass-half', 'bg-navy'],
        [r.filter(isOverdue).length, 'Quá hạn', 'fa-triangle-exclamation', 'bg-danger'],
        [r.filter(isDueToday).length, 'Hôm nay', 'fa-clock', 'bg-warning'],
        [r.filter((f) => f.status === 'Completed').length, 'Hoàn tất', 'fa-circle-check', 'bg-success']
      ]; }, [rows]);
      ...
      const onAction = (action, f) => {
        if (action === 'wa') { const n = String(f.leadPhone || '').replace(/\D/g, ''); if (n) window.open('https://zalo.me/' + n, '_blank'); }
          ...
          gsRun('updateFollowUp', { id: f.id, status: 'Completed' }, currentUser).then((res) => {
            if (res && res.success) { Swal.fire({ icon: 'success', title: 'Đã hoàn tất lịch chăm sóc!', timer: 1500, showConfirmButton: false }); mutate(); swrMutate('dash:stats'); }
          ...
          Swal.fire({ icon: 'warning', title: 'Xóa lịch chăm sóc #' + f.id + '?', text: 'Hành động này không thể hoàn tác!', showCancelButton: true, confirmButtonColor: '#ea4335', confirmButtonText: 'Xóa', cancelButtonText: 'Hủy' })
            .then((r) => { if (r.isConfirmed) gsRun('deleteFollowUp', f.id, currentUser).then((res) => {
              if (res && res.success) { Swal.fire({ icon: 'success', title: res.message, timer: 1800, showConfirmButton: false }); mutate(); swrMutate('dash:stats'); }
              else Swal.fire({ icon: 'error', title: 'Lỗi', text: (res && res.message) || 'Không thể xóa' }); }); });
        }
      };
      ...
      const tableRef = useDataTable('fuTable', rows === undefined ? null : visible, () => ({
        search: { search: filters.search },
        columns: [
          { data: 'leadName', title: 'Khách hàng', render: (d, t, f) => '<strong>' + esc(d) + '</strong><br><small style="color:#789">' + esc(f.leadPhone || '') + '</small>' },
          { data: 'type', title: 'Loại', render: (d, t) => t === 'display' ? '<i class="fas ' + (FU_ICONS[d] || 'fa-circle') + '" style="color:var(--navy-accent);margin-right:6px"></i>' + esc(viEnum(d) || d) : d },
          { data: 'notes', title: 'Ghi chú', render: (d) => esc(String(d || '').substr(0, 70)) + (String(d || '').length > 70 ? '…' : '') },
          { data: 'dueAt', title: 'Hạn chót', render: (d, t, f) => t === 'display'
              ? (d ? fmtDT(d) + (isOverdue(f) ? ' ' + badge('Overdue') : '') : '<small style="color:#999">đã ghi nhận</small>') : (d || '') },
          { data: 'status', title: 'Trạng thái', render: (d, t) => t === 'display' ? badge(d) : d },
          { data: 'assignedAgent', title: 'Nhân viên', render: (d) => esc(d || '—') },
          { data: null, title: 'Thao tác', orderable: false, className: 'dt-actions actions-4', width: '140px', render: (d, t, f) => `<div class="table-actions slots-4">
            ${canEdit && f.status === 'Pending' ? '<button class="action-icon view-icon" data-action="done" title="Đánh dấu hoàn tất"><i class="fas fa-check"></i></button>' : '<span class="action-slot" aria-hidden="true"></span>'}
            ...
            ${canEdit ? '<button class="action-icon edit-icon" data-action="edit" title="Sửa"><i class="fas fa-edit"></i></button>' : ''}
        ...
        <div className="followups-page-wrapper">
          ...
          <div className="mob-pipeline-bar" style={{ marginBottom: 10 }}>
            <div className="mob-pills-scroll">
              {FU_STAGES.map((s) => (
                <button
                  key={s.key}
                  className={'mob-pill ' + (stage === s.key ? 'active' : '')}
                  onClick={() => setStage(s.key)}
                >
                  <span>{s.label}</span>
                  <span className="mob-pill-count">{counts[s.key] || 0}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="filters-section desk-filters-section">
            ...
            <div className="filters-grid">
              <div className="filter-group">
                <label><i className="fas fa-magnifying-glass"></i> Tìm kiếm</label>
                <input className="filter-input" value={filters.search} placeholder="Khách hàng, ghi chú, nhân viên…" onChange={(e) => setFilters({ ...filters, search: e.target.value })} />
              </div>
              <SearchableDropdown label="Loại tương tác" icon="fas fa-list" options={opts(ENUMS.followUpType)} value={filters.type} onChange={(v) => setFilters({ ...filters, type: v })} placeholder="Tất cả loại" />
              {all && <SearchableDropdown label="Nhân viên phụ trách" icon="fas fa-user-tie" options={(lookups.agents || []).map((a) => ({ value: a.username, label: a.username + ' (' + a.role + ')' }))} value={filters.agent} onChange={(v) => setFilters({ ...filters, agent: v })} placeholder="Tất cả nhân viên" />}
            </div>
          </div>
            ...
            <div className="mob-filter-sheet-overlay" onClick={() => setShowFilterDrawer(false)}>
              <div className="mob-filter-sheet" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="mob-sheet-body">
                  ...
                  {all && (
                    <div className="form-group" style={{ marginBottom: 12 }}>
                      <SearchableDropdown label="Nhân viên phụ trách" icon="fas fa-user-tie" options={(lookups.agents || []).map((a) => ({ value: a.username, label: a.username + ' (' + a.role + ')' }))} value={filters.agent} onChange={(v) => setFilters({ ...filters, agent: v })} placeholder="Tất cả nhân viên" />
                    </div>
                  )}
                </div>
                <div className="mob-sheet-footer">
            ...
            <div className="mob-leads-cards-container mobile-only-cards">
              {loading ? (
                ...
                <div className="mob-empty-state">
                  ...
                  <p>Không có lịch chăm sóc nào phù hợp</p>
                </div>
              ) : (
                <div className="mob-followups-list">
                  {visible.map((f) => {
                    const overdue = isOverdue(f);
                    const dueToday = isDueToday(f);
                    const cleanPhone = String(f.leadPhone || '').replace(/\D/g, '');
                    return (
                      <div key={f.id} className={'mob-followup-card ' + (overdue ? 'is-overdue' : dueToday ? 'is-today' : '')}>
                        {/* HÀNG 1: Avatar + Tên khách + Loại + Trạng thái */}
                        <div className="mob-fu-head">
                          <div className="mob-fu-lead-info" onClick={() => setViewingLead(f)}>
                            <div className="mob-fu-avatar" style={{ background: getLeadAvatarColor(f.leadName || 'U') }}>
                              {getLeadInitials(f.leadName || 'U')}
                            </div>
                            <div className="mob-fu-lead-text">
                              <span className="mob-fu-lead-name">{f.leadName || 'Khách hàng #' + f.leadId}</span>
                              <span className="mob-fu-lead-phone">{f.leadPhone || 'Chưa có SĐT'}</span>
                            </div>
                          </div>
                          <div className="mob-fu-badges">
                            <span className="mob-fu-type-badge">
                              {f.type === 'Zalo' ? (
                                <svg className="zalo-logo-img" viewBox="0 0 100 100" style={{ width: 13, height: 13, marginRight: 4, verticalAlign: '-1px' }}>
                                  <circle cx="50" cy="50" r="47" fill="#ffffff" stroke="#008fe5" strokeWidth="4.5" />
                                  <path d="M 50 15 C 69.33 15 85 30.67 85 50 C 85 69.33 69.33 85 50 85 C 44.2 85 38.7 83.6 33.8 81.1 L 18 86.5 L 22.8 72.3 C 17.9 66.2 15 58.4 15 50 C 15 30.67 30.67 15 50 15 Z" fill="#008fe5" />
                                  <text x="50.5" y="58" fill="#ffffff" fontFamily="system-ui, sans-serif" fontSize="28" fontWeight="900" textAnchor="middle" letterSpacing="-1.2">Zalo</text>
                                </svg>
                              ) : (
                                <i className={'fas ' + (FU_ICONS[f.type] || 'fa-circle')}></i>
                              )} {viEnum(f.type) || f.type}
                            </span>
                            <Badge s={f.status} />
                          </div>
                        </div>

                        {/* HÀNG 2: Thời hạn & Người phụ trách */}
                        <div className="mob-fu-meta-row">
                          <div className={'mob-fu-due-pill ' + (overdue ? 'overdue' : dueToday ? 'today' : '')}>
                            <i className={'fas ' + (overdue ? 'fa-triangle-exclamation' : 'fa-clock')}></i>
                            <span>{f.dueAt ? fmtDT(f.dueAt) : 'Đã ghi nhận hoạt động'}</span>
                            {overdue && <span className="mob-fu-overdue-tag">Quá hạn</span>}
                          </div>
                          {f.assignedAgent && (
                            <div className="mob-fu-agent-pill">
                              <i className="fas fa-user-tie"></i> {f.assignedAgent}
                            </div>
                          )}
                        </div>

                        {/* HÀNG 3: Nội dung ghi chú chăm sóc */}
                        {f.notes && (
                          <div className="mob-fu-notes-box">
                            <i className="fas fa-quote-left"></i>
                            <p>{f.notes}</p>
                          </div>
                        )}

                        {/* HÀNG 4: Thanh nút hành động 1-chạm */}
                        <div className="mob-fu-actions">
                          {cleanPhone && (
                            <a href={'tel:' + cleanPhone} className="mob-fu-btn call" title="Gọi điện">
                              <i className="fas fa-phone"></i> <span>Gọi</span>
                            </a>
                          )}
                          {cleanPhone && (
                            <button className="mob-fu-btn zalo" onClick={() => onAction('wa', f)} title="Nhắn Zalo">
                              <svg className="zalo-logo-img" viewBox="0 0 100 100" style={{ width: 14, height: 14, marginRight: 4 }}>
                                <circle cx="50" cy="50" r="47" fill="#ffffff" stroke="#008fe5" strokeWidth="4.5" />
                                <path d="M 50 15 C 69.33 15 85 30.67 85 50 C 85 69.33 69.33 85 50 85 C 44.2 85 38.7 83.6 33.8 81.1 L 18 86.5 L 22.8 72.3 C 17.9 66.2 15 58.4 15 50 C 15 30.67 30.67 15 50 15 Z" fill="#008fe5" />
                                <text x="50.5" y="58" fill="#ffffff" fontFamily="system-ui, sans-serif" fontSize="28" fontWeight="900" textAnchor="middle" letterSpacing="-1.2">Zalo</text>
                              </svg>
                          ...
                          {canEdit && f.status === 'Pending' && (
                            <button className="mob-fu-btn done" onClick={() => onAction('done', f)} title="Đánh dấu xong">
                              <i className="fas fa-circle-check"></i> <span>Xong</span>
                            </button>
                          )}
                          {canEdit && (
                          ...
                          {canDel && (
                            <button className="mob-fu-btn del" onClick={() => onAction('delete', f)} title="Xóa">
